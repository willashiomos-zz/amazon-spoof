...
function adobeAnalytics() {
  ...
  $("#nav-primary a").click(function (e) {
    pushAdobeTagsMain("navigation click", "link", {
      name: "header:" + $(this).text(),
    });
  });
  ...
  $("footer .links a, footer .privacy a").click(function (e) {
    if (!$(this).hasClass("footer-brand")) {
      pushAdobeTagsMain("navigation click", "link", {
        name: "footer:" + $(this).text().replace("- from Footer", ""),
      });
    }
  });
  ...
  $("footer .icons a").click(function (e) {
    pushAdobeTagsMain("navigation click", "link", {
      name: "footer:" + $(this).attr("id").replace("-link-footer", ""),
    });
  });
  ...
  if ($("#about-events").length) {
    $("#event-location").change(function (e) {
      pushAdobeTagsNoSub({
        event: "event location select",
        eventLocation: $(this).val(),
      });
    });
  }
...
function pushAdobeTagsMain(eventName, name, obj) {
  ...
  console.log(
    "debuug dataLayerArry affter push = " +
      JSON.stringify(window.dataLayerArray)
  );
}
...
function globalEvents() {
  ...
  $("#country-link-list a").click(function (e) {
    var langMenuData = {
      "/entrepot/montreal-emplois":
        "https://www.amazondelivers.jobs/warehouse-jobs/montreal-jobs/",
      "/covid-19-faqs": "https://quebec.amazondelivers.jobs/covid-19-updates/",
      "/warehouse-jobs/montreal-jobs":
        "https://quebec.amazondelivers.jobs/entrepot/montreal-emplois/",
  ...
  $("#introductory > div").append(
    '<a href="#scroll-to-content" class="link-explore"><span></span></a>'
  );

  $('a[href$="#scroll-to-content"]').click(function (e) {
    $("html,body").animate(
      { scrollTop: $("#scroll-to-content").offset().top - fixHeight },
      "1000",
      function () {}
    );
  });
  ...
  $(".scroll-to-content nav a").click(function (e) {
    ...
    $("html,body").animate(
      { scrollTop: $(targetID).offset().top - scrollHeight },
      1000,
      "easeOutCirc"
    );
...
function video() {
  ...
  if ($("a.video").length) {
    $.getScript(
      "//clientfiles.tmpwebeng.com/js/lib/fancybox.js?v=2.1.3",
  ...
  if ($(".video-container").length > 0) {
    $(".video-container a").click(function (e) {
      ...
      if (videoSrc.indexOf(".mp4") != -1) {
        var embedVideo =
          '<video controls="" style="width:100%;height:100%;"   autoplay="true" name="media"><source src="' +
          videoSrc +
          '" type="video/mp4"></video>';
        $(this).parent().append(embedVideo);
      } else {
        var embedFrame =
          '<iframe allowfullscreen="" style="width:100%;height:100%;"  frameborder="0" src="' +
          $(this).attr("href") +
          '"></iframe>';
      ...
      if ($(this).next().prop("tagName") == "DIV") {
        ...
        if (currPosition === "absolute") {
          ...
          if (!$(this).next().find(".head").length) {
            $(this)
              .next()
              .prepend('<span class="head">' + $(this).text() + "</span>");
          }
          ...
          $("html,body").animate(
            { scrollTop: $("article").offset().top - fixHeight },
            "1000",
            function () {}
          );
...
function setLocationDrop() {
  if ($("ul.home-location").length) {
    ...
    $("ul.home-location").each(function (i) {
      ...
      $(this).after(
        '<button type="submit" id="btn-location-' +
          i +
          '"> Explore jobs </button>'
      );
      $(this).each(function () {
      ...
      $("#btn-location-" + i).click(function (e) {
        ...
        if (selLocation == initialDropText) {
          ...
          ga(
            ...
            "Click",
            "Home - Explore Jobs - " +
              $("#sel-location-" + i + "  option:selected").text()
          );
    ...
    $.ajax({
      url:
        "https://clientfiles.tmpwebeng.com/amazon/feed/amazon-home-dd.aspx?feed=sdydfjgd",
      type: "GET",
...
function externalLink() {
  ...
  $("a").each(function () {
    ...
    if (
      this.href != "" &&
      !a.test(this.href) &&
      $(this).attr("href").indexOf("amazontb.runmytests.com") == -1 &&
      $(this).attr("href").indexOf("search.amazondelivers") == -1 &&
      $(this).attr("href").indexOf("mailto") == -1 &&
      $(this).attr("data-open") != "self"
    ) {
    ...
    if (
      $(this).attr("href").indexOf("amazon.force.com") != -1 ||
      $(this).attr("href").indexOf("search.amazondelivers.jobs") != -1 ||
      $(this).attr("href").indexOf("amazonrefresh.runmytests.com") != -1
    ) {
  ...
  $("a:contains('Join Amazon's Talent Community')")
    .parent()
    .after(
      '<p style="font-size:0.8rem;">"STOP" to opt-out. T&C/Privacy Policy <a target="_blank" rel="noopener"  href="https://www.sms-terms.com/amazon/" aria-label="click here for Privacy Policy">here</a>.</p>'
    );
}
  ...
  if (eventTrackLabel.length > 0) {
    $("article a").click(function () {
      ga(
        ...
        "Custom Event",
        "Click",
        eventTrackLabel + " - " + $(this).text().replace(" >", "")
      );
    });
...
function homePopup() {
  if ($("ul#home-location").length) {
    $("ul#home-location")
      ...
      .replaceWith(
        '<label for="sel-location">' +
          $("ul#home-location").prev().html() +
          "</label>"
      );

    $("ul#home-location").after(
      '<button type="submit" id="btn-location" > Go </button>'
    );
  ...
  if ($("body#covid-19-updates").length) {
    $(".accordion h2").click(function () {
      ...
      if (h2Txt.indexOf("Is Amazon hiring during") != -1) {
        ...
        h2Txt.indexOf("What is Amazon doing to provide a safe") != -1
      ) {
        gaLabel = "Employee safety";
      } else if (
        h2Txt.indexOf("Can I work for Amazon if there is a Curfew") != -1
      ) {
        gaLabel = "Shelter in place";
      } else if (
        h2Txt.indexOf("What is Amazon doing to help employees") != -1
      ) {
        gaLabel = "Employee help";
      } else if (
        h2Txt.indexOf("I’m laying off my employees because of COVI") != -1
      ) {
